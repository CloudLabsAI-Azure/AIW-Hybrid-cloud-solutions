...
- [Access an Azure Arc-enable VM over SSH without direct connectivity enabled](#task-2-ssh-access-to-azure-arc-enabled-servers)
...
1. Open yje HybridHost001 desktop open the Azure Portala Install the SSH az CLI extension. Run the following command ```az extension add --name ssh```. To check if the extentsion was successfully installed run ```az extension show --name ssh```.
...
1. SSH into Azure Arc Servers. Users can login using AAD issued certificates or using local user credentials. Run ```az ssh arc --resource-group hybridhost --vm-name vm002 --local-user demouser``` to establisg a SSH session into vm002.
